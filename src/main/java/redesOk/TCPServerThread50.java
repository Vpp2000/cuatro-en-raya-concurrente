...
public class TCPServerThread50 extends Thread{
    ...
    public void run() {
        ...
        try {
            try {               
                ...
                while (running) {
                    message = in.readLine();
                    //tcpserver.jugada==this.clientID
                    if (message != null && messageListener != null && tcpserver.jugada+1==this.clientID && tcpserver.nrcli==tcpserver.max_clients) {
    ...
    public void sendMessage(String message){//funcion de trabajo
        if (mOut != null && !mOut.checkError()) {
            ...
            mOut.flush();
       if(tcpserver.terminado==true) this.stopClient();
        }
    }
